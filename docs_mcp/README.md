...
- `guias/` — agent workflows (`agent-workflow`), brownfield Forge MCP (`brownfield-forge-mcp`), C4 / Archify MCP (`c4-architecture-mcp`), consuming the Docs MCP (`consumir-docs-mcp`), graph tool catalog (`graph-tools-catalog`), safe refactor (`refactor-seguro`).
