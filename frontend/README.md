...
- `/repos/:id/edit` — Editar repo: credencial, branch, webhook secret (Bitbucket) y **alcance del índice** (todo el repo vs restringido: `indexIncludeRules` → `repositories.index_include_rules`). `PATCH /repositories/:id`.
