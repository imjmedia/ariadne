...
async function reviseCodeWithLlm(state: RefactorState): Promise<Partial<RefactorState>> {
  ...
  try {
    ...
    if (!code) {
      return { error: 'LLM no configurado o respuesta vacía del modelo (Ajustes → Proveedores IA)', approved: false };
    }
    return {
      proposedCode: code,
...
async function generateTests(state: RefactorState): Promise<Partial<RefactorState>> {
  ...
  if (!hasOrchestratorLlmConfigured()) {
    const lines = [
      `// Tests sugeridos para ${state.nodeId} (generar con API key en Ajustes para código completo)`,
