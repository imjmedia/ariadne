/**
 * @fileoverview Cypher para :System, :Container, PART_OF (File), COMMUNICATES_WITH.
 */
...
export function buildC4IngestCypher(
  ...
  const catchall =
    spec.containers.find((c) => c.key === '_unassigned') ??
    spec.containers.find((c) => c.key === 'application');
  if (catchall) {
  ...
  for (const comm of spec.communications ?? []) {
    const fk = cypherSafe(comm.fromKey);
    const tk = cypherSafe(comm.toKey);
    const lbl = cypherSafe(comm.label ?? 'depends_on');
    merge.push(
      `MATCH (a:Container {projectId: ${pid}, repoId: ${rid}, key: ${fk}}), (b:Container {projectId: ${pid}, repoId: ${rid}, key: ${tk}}) MERGE (a)-[r:COMMUNICATES_WITH]->(b) SET r.reason = ${lbl}`,
    );
  }
