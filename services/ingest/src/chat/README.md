...
- **`chat-schema-question.util.ts`** — Re-exporta detectores desde **`ariadne-common`** (`wantsSchemaDatabaseQuestion`, `wantsReengineeringQuestion`, `wantsArchitectureDomainQuestion`) + Cypher de esquema local. Fast-path esquema solo si el **router** (orchestrator) o heurística lo clasifican como `schema_database`; preguntas de arquitectura/desacoplamiento → agente **reingeniería**.
...
- **`llm-unified.ts`** — `LLM_MODEL_INGEST` + API key desde Ajustes (`active-llm-config`).
...
Si **`ORCHESTRATOR_URL`** está definido (p. ej. `http://orchestrator:3001` en Docker), **`POST .../chat`** **no ejecuta LLM en ingest**: hace proxy a **`POST /codebase/chat/repository|project/...`** del orchestrator (LangGraph: retrieve → synthesize). Preguntas de **endpoints Strapi no usados en el front** se resuelven en orchestrator con early-return → **`POST /internal/projects|repositories/.../unused-api-endpoints`** (tablas por tier, sin sintetizador genérico). Preguntas de **esquema/diagrama de base de datos** (`wantsSchemaDatabaseQuestion`) hacen early-return → **`POST /internal/projects|repositories/.../schema-database`** (esquema real Prisma/TypeORM/Strapi, excluye DTOs del frontend; tabla + Mermaid `erDiagram`). Con **`responseMode: evidence_first`**, el orchestrator pide el JSON MDD a **`POST /internal/repositories/:repoId/mdd-evidence`** (mismas cabeceras internas). Con **`responseMode: raw_evidence`** + **`deterministicRetriever: true`**, el orchestrator delega el retrieve a **`POST /internal/repositories/:repoId/raw-evidence-deterministic`** (sin ReAct LLM); el primer paso **`get_graph_summary`** usa **`evidenceVerbosity: full`** y por tanto **`CHAT_RAW_EVIDENCE_GRAPH_SAMPLE_CAP`** para no volcar miles de paths en el JSON de evidencia. El ingest expone **`POST /internal/repositories/:repoId/retriever-tool`** para que el orchestrator ejecute Cypher/RAG/archivos sin duplicar la capa de datos (red Docker interna, sin API key).
...
- **`POST /projects/:projectId/chat`** — Mismo body. **Multi-root (varios repos):** por defecto (`strictChatScope` omitido o `true`) exige `scope` explícito o inferencia por **roles** en `project_repositories`; si no, respuesta `[AMBIGUOUS_SCOPE]` con candidatos. `strictChatScope: false` permite chat amplio sobre todos los roots. **Endpoints backend sin uso:** preguntas tipo «qué endpoints del back no se usan en el front» → respuesta temprana con tablas Strapi (`StrapiRoute`) y/o Nest (`NestRoute`; con orchestrator vía **`POST /internal/projects/:id/unused-api-endpoints`**). **Esquema/diagrama de BD:** preguntas tipo «diagrama de base de datos», «esquema», «modelo de datos», «ERD», «content-types» → respuesta temprana `buildSchemaDatabaseResponse` con `:Model` (source `prisma`/`typeorm`), `:StrapiContentType` y `RELATES_TO` + Mermaid `erDiagram`; **excluye** DTOs del frontend (`src/Models/*`). En multi-repo, la inferencia por roles (`resolve-chat-scope-from-message.util.ts`) mapea señales de BD al repo **backend**. Límite `CHAT_SCHEMA_MAX` (default 5000). **Preflight:** si el retrieval mezcla `repoId` y el mensaje incluye una ruta que resuelve a un único repo (`resolveRepositoryForWorkspacePath`), se recorta contexto antes del sintetizador (ver `CHAT_PREFLIGHT_PATH_REPO`). **Listados íntegros:** si el mensaje pide volcado completo de componentes o inventario indexado, respuesta temprana con tablas Markdown (límites `CHAT_COMPONENT_FULL_MAX`, `CHAT_GRAPH_INVENTORY_FULL_MAX`; detectores en `chat.constants.ts`).
- **`POST .../modification-plan`** — Body: `{ userDescription, scope?, currentFilePath?, questionsMode?: 'business'|'technical'|'both' }`. Multi-root: ancla con un solo `scope.repoIds`, `currentFilePath` absoluto bajo el clone, o `MODIFICATION_PLAN_LEGACY_FIRST_REPO=true`. Utilidades: `modification-plan-*.util.ts`, exclusiones por env `MODIFICATION_PLAN_*`. Tras hits por término/embedding, **expande 1 hop** por `CALLS`/`IMPORTS`/`RENDERS` y rankea por impacto (`ModificationPlanEvidenceService`). Respuesta incluye `graphEvidenceBundle` + `changePlanTemplate` (tasks con `phase`/`criterion`/`evidence`/`dependsOn`). Caps: `MODIFICATION_PLAN_MAX_FILES`, `MODIFICATION_PLAN_EVIDENCE_MAX_FILES`, `MODIFICATION_PLAN_IMPACT_SEED_LIMIT`.
- **`POST /internal/.../integration-handoff-plan`** — Plan multi-query para handoffs NEW-LEG (orchestrator `ChatIntegrationHandoffAgent`): parsea AC, queries UX (`catálogo`+`previsualizador`, `costos asociados`+`pauta`), patrones de path/component en Cypher, merge con penalización de módulo único (ej. solo DataCamiones). Cap: `INTEGRATION_HANDOFF_PLAN_MAX_FILES` (default 40). Ver `integration-handoff-plan.util.ts` en **ariadne-common**.
- **`POST /projects/:projectId/validate-change-plan`** — Gate 2 sobre `ChangePlan`.
- **`POST /projects/:projectId/validate-tasks-json`** — Gate 2 post-deliverable: mapea Forge `tasksJson` → `ChangePlan` y valida (bloquea si `BLOCKED`).
...
- **`POST /internal/repositories/:repoId/mdd-evidence`** — Body: `{ message, gatheredContext, collectedResults, projectScope?, projectId? }` — JSON **MDD** para `ask_codebase` / LegacyCoordinator; inyecta lecturas mínimas si el contexto está vacío pero hay archivos en índice.
- **`POST /internal/projects/:projectId/mdd-evidence-merged`** — MDD fusionado de todos los roots (snapshots post-sync o live por repo) + `multi_root` completo.
- **`POST /internal/projects/:projectId/brownfield-parity-pack`** — Parity pack Forge con `mergeMode: project_multi_root` (ver `brownfield/README.md`).
...
**Requisitos LLM (todo via OpenRouter):** API key en **Ajustes → Proveedores IA** (admin). Modelo global: `LLM_CHAT_MODEL` (default: `google/gemini-2.0-flash-001`). Per-componente: `LLM_MODEL_INGEST` para este servicio, `ORCHESTRATOR_LLM_MODEL` para el orquestador.

| Variable | Requerida | Default | Descripción |
|---|---|---|---|
| *(Ajustes UI)* | **Sí** | — | API key LLM (Postgres cifrado) |
...
Volcado **casi completo** por defecto (OpenAPI ops, Model, **StrapiContentType / StrapiRoute**, NestService, `evidence_paths`, texto de consulta en `summary`, opcional **`multi_root`**). Prioridad API: `OpenApiOperation` → `StrapiRoute` → cliente frontend (`apiDirection`) → `NestController` (AST). Entidades: `Model` (Prisma/TypeORM) + `StrapiContentType` (`source: strapi`) + interfaces `src/Models` (`source: frontend`). **`multi_root`:** composición del workspace Ariadne (`repositories[]` con slug, rol, sync), alcance del snapshot (`mdd_scope_repo_ids`, `is_primary`), conteos cross-repo Falkor (`CALLS_API`, `CALLS_STRAPI_ROUTE`, …) y `notes` que distingue multi-repo Git vs topología de deploy. Construcción: `mdd-multi-root.util.ts` + inyección desde `ChatService.buildMddEvidenceForRepository`. **`POST …/mdd-evidence`** resuelve siempre el `projectId` Falkor del workspace vía `resolveProjectIdForRepo`. Si el grafo directo no devuelve nodos Strapi, reintenta vía `(File)-[:CONTAINS]->(StrapiContentType|StrapiRoute)`. Si sigue vacío pero hay paths Strapi en evidencia, **`mdd-strapi-path-fallback.ts`** (schemas + `createCoreRouter` + custom routes). Repos SPA React/Vite: **`mdd-frontend-path-fallback.ts`** (`src/Models`, `src/api`, `src/api/queries`, `apiDirection`). **`business_logic`** se rellena desde paths de servicios/módulos aunque entidades y rutas vengan del grafo Falkor. Acota con env si Falkor o el payload JSON son demasiado grandes:

**`openapi_spec`:** además de `found` / `path` (`File.openApiTruth`) / `trust_level`, el builder puede rellenar **`swagger_dependencies`**, **`swagger_related_paths`**, **`supplementary_doc_paths`** (Markdown inventario/manual API en `evidence_paths`) y **`supplementary_docs`** (extracto vía `getFileSnippet`, caps `MDD_MAX_SUPPLEMENTARY_DOCS` / `MDD_SUPPLEMENTARY_DOC_EXCERPT_CHARS`) y **`notes`** cuando hay Swagger en código pero no hay spec OpenAPI indexada o faltan nodos `OpenApiOperation`/`NestController`.
...
Implementación: `mdd-limits.ts`, `mdd-document.builder.ts`, `mdd-document.types.ts`, `mdd-multi-root.util.ts`.
