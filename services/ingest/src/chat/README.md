...
Volcado **casi completo** por defecto (OpenAPI ops, Model, **StrapiContentType / StrapiRoute**, NestService, `evidence_paths`, texto de consulta en `summary`). Prioridad API: `OpenApiOperation` → `StrapiRoute` → `NestController` (AST). Entidades: `Model` (Prisma/TypeORM) + `StrapiContentType` (`source: strapi`). **`POST …/mdd-evidence`** resuelve siempre el `projectId` Falkor del workspace vía `resolveProjectIdForRepo` (no confía en `body.projectId` del orchestrator). Si el grafo no tiene nodos Strapi pero `evidence_paths` incluye `schema.json` / `routes/*`, **`mdd-strapi-path-fallback.ts`** infiere entidades y contratos leyendo esos archivos. Acota con env si Falkor o el payload JSON son demasiado grandes:
