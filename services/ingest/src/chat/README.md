...
- **`chat-schema-question.util.ts`** — Detector `wantsSchemaDatabaseQuestion` (esquema/diagrama/tablas/ERD/content-types) + Cypher de esquema real: `:Model` con `m.source IN ['prisma','typeorm']` (excluye DTOs del frontend), `:Enum`, `:StrapiContentType` y `RELATES_TO`. Fast-path en `runUnifiedPipeline` → `buildSchemaDatabaseResponse` (tabla + Mermaid `erDiagram`); espejo en orchestrator vía `POST /internal/…/schema-database`.
