...
const API_MODULE_PATH_RE = /\/api\/[^/]+\.(tsx?|ts)$/i;
...
export function isFrontendEvidencePath(path: string): boolean {
  return MODEL_PATH_RE.test(path) || API_MODULE_PATH_RE.test(path);
}
...
export async function inferFrontendMddFromEvidencePaths(params: {
  ...
  for (const p of params.evidencePaths) {
    ...
    if (API_MODULE_PATH_RE.test(p)) {
      const base = p.split('/').pop()?.replace(/\.(tsx?|ts)$/, '') ?? p;
      modules.add(`frontend:${base}`);
    }
  ...
  const business_logic = [...modules].slice(0, 200).map((service) => ({
    service,
