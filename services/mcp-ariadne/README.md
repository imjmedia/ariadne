...
- **ask_codebase** — Preguntas en NL; ingest u orchestrator. Parámetro **`responseMode`:** `default` (prosa + ReAct, como “Chat normal” en The Forge UI), **`evidence_first`** (JSON MDD 7 claves, como “MDD/SDD” en la UI), **`raw_evidence`** (JSON de evidencia; con **`deterministicRetriever: true`** sin LLM en retrieve — “Evidencia bruta” en la UI). **Si omites `responseMode`**, el MCP envía **`raw_evidence`** + **`deterministicRetriever: true`** (barato por defecto para agentes; **no** equivale a omitir `responseMode` en el frontend The Forge, que cae en `default`). Opcionales: **`scope`**, **`twoPhase`**. Con `raw_evidence`, **`deterministicRetriever`** default **true**; pon **false** para ReAct+LLM en retrieve. Timeouts: **`MCP_ASK_CODEBASE_TIMEOUT_MS`** (default 300s, 900s con `raw_evidence`). Progreso en log: **`MCP_ASK_CODEBASE_PROGRESS_LOG_MS`** (ver arriba). Requiere INGEST_URL. **Routing:** preferir herramientas puntuales si ya conoces path/símbolo; en multi-root **`scope`** o `roots[].id` como `projectId`. Detalle: `docs/notebooklm/mcp_server_specs.md` → **Tool: `ask_codebase`** y tabla *Modo The Forge*.
